...
std::vector<std::string> ParseNalUnit(char* pFrameData, int nFrameLength)
{
	...
	char                     naluHeader[4] = { 0x00, 0x00, 0x01 };  //不找00000001
	int                      naluHeaderLen = 3;
...
char* nalUnitGet(NaluUint_t* nalu, int sepLength, char* data, int& length)
{
	if (length <= 0) {
		nalu->size = 0;
		return nullptr;
	...
	for (; i < length; i++)
	{
		if (ptr[i] == 0x01 && ptr[i-1] == 0x00 && ptr[i-2] == 0x00 && ptr[i-3] == 0x00) {
			break;
		}
	}
	nalu->data = ptr;
	nalu->type = ptr[sepLength];
	// 最后一个
	if (i == length)
		nalu->size = length;
	else
		nalu->size = (i + 1) - sepLength;

	length -= nalu->size;
	return ptr + nalu->size;;
}