...
// 8000 1 16
bool AVfaac::init(int nSamplePerSec, int nChn, int nBitsPerSample, int nOfmt)
{
...
// 编码 返回编码后数据长度
unsigned char* AVfaac::encode(unsigned char* pcmData, long dataLen, int& accLen)
{
	if (NULL == _faacEnc) {
		return 0;
	}
...
unsigned char* AVfaac::specialData(int* len)
{
	*len = _nSpecialLen;
